...
4. **Syslog**: consoleLog, authPrivFrom/To 등 적용.
...
- **한 번에 적용**: `apply_basic_settings_tool(hostname=..., nameservers=..., ntp_servers=..., timezone=..., admin_password=..., root_password=..., ...)`  
  넘긴 인자만 적용되며, 생략한 항목은 건너뛴다. 내부에서 위 순서대로 실행됨.
...
### 자동 생성 로직
1. **Primary/Secondary device**: configsyncIp, mirrorIp, unicastAddress PATCH.
2. **add-to-trust**: Primary에서 Secondary를 trust 도메인에 추가.
3. **Device Group**: 생성 후 두 device를 group에 추가.
4. **config-sync**: to-group 실행.
5. **HA 상태**: `get_ha_status_tool()`로 Sync 상태 확인.
